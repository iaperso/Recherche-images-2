...
function decode(v:string){return v.replace(/&amp;/g,'&').replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/\\u0026/g,'&').replace(/\\u002F/gi,'/').replace(/\\u003A/gi,':').replace(/\\x2F/gi,'/').replace(/\\x3A/gi,':').replace(/\\\//g,'/')}
...
function contentImage(url:string){try{const u=new URL(url.replace(/^http:/i,'https:'));if(!/^https:$/.test(u.protocol)||!cdnImage(u.toString()))return false;const s=(u.pathname+u.search).toLowerCase();if(/\.png(?:$|\?)/i.test(s))return false;if(/[?&]ava=1(?:&|$)/i.test(s))return false;if(/(?:logo|icon|emoji|sprite|blank|camera)/i.test(s))return false;return /\.(?:jpe?g|webp)(?:$|\?)/i.test(s)||/\/ig\d?\//i.test(s)||/userapi/i.test(u.hostname)}catch{return false}}
...
function decodeSearchUrl(raw:string){let v=decode(raw);for(let i=0;i<3;i++){if(!/%[0-9a-f]{2}/i.test(v))break;try{const n=decodeURIComponent(v);if(n===v)break;v=n}catch{break}}return decode(v)}
...
async function searchVkVideoLive(q:string,page:number){
 ...
 const posts:Post[]=channels.map((channel:any,index:number)=>{
  ...
  return{uri:`vk-live:${handle}:${streamId}`,cid:`vklive-${streamId}`,text:title,createdAt:safeIso(stream?.startedAt||channel?.lastStreamAt),author:{handle,displayName:String(channel?.nick||handle),avatar:channel?.avatarUrl||null},video:{playlist:null,player:null,pageUrl,thumbnail:thumb,alt:title,aspectRatio:null},likeCount:Number(stream?.counters?.likes||0),repostCount:0}
 })
...
function searchPageUrls(q:string,kind:DirectKind,page:number){
 const section=kind==='image'?'photos':'videos';const offset=Math.max(0,page)*40
 const base=[`https://vk.com/search?c%5Bq%5D=${encodeURIComponent(q)}&c%5Bsection%5D=${section}&c%5Boffset%5D=${offset}`,`https://m.vk.com/search?c%5Bq%5D=${encodeURIComponent(q)}&c%5Bsection%5D=${section}&c%5Boffset%5D=${offset}`]
 if(kind==='image'){
  const site=`site:vk.com ${q}`
  base.push(`https://www.bing.com/images/search?q=${encodeURIComponent(site)}&first=${1+page*35}&safeSearch=Off`)
  base.push(`https://www.bing.com/images/search?q=${encodeURIComponent(`site:vk.com/photo ${q}`)}&first=${1+page*35}&safeSearch=Off`)
  base.push(`https://search.brave.com/search?q=${encodeURIComponent(`${site} photo`)}&source=web&offset=${Math.max(0,page)}`)
  base.push(`https://yandex.com/images/search?text=${encodeURIComponent(site)}&p=${Math.max(0,page)}`)
  base.push(`https://www.google.com/search?tbm=isch&safe=off&q=${encodeURIComponent(site)}&start=${Math.max(0,page)*20}`)
 }else{
  base.push(`https://www.bing.com/videos/search?q=${encodeURIComponent(`site:vk.com ${q} video`)}&first=${1+page*35}&safeSearch=Off`)
  base.push(`https://www.bing.com/videos/search?q=${encodeURIComponent(`site:vkvideo.ru ${q}`)}&first=${1+page*35}&safeSearch=Off`)
  base.push(`https://search.brave.com/search?q=${encodeURIComponent(`site:vk.com ${q} video`)}&source=web&offset=${Math.max(0,page)}`)
  base.push(`https://search.brave.com/search?q=${encodeURIComponent(`site:vkvideo.ru ${q}`)}&source=web&offset=${Math.max(0,page)}`)
 }
 return base
}
...
function extractImageUrls(html:string){
 ...
 const add=(raw:string)=>{const u=clean(decodeSearchUrl(raw)).replace(/^http:/i,'https:');if(contentImage(u)&&!seen.has(u)){seen.add(u);out.push(u)}}
 for(const m of text.matchAll(/(?:murl|mediaurl|contentUrl|original|originalUrl|imageUrl)["'=: ]+\s*["'](https?:\\?\/\\?\/[^"'<>]+)["']/gi))add(m[1])
 for(const m of text.matchAll(/https?%3A%2F%2F[^\s"'<>]+/gi))add(m[0])
...
export async function searchVkDirect(q:string,kind:DirectKind,page=0){
 if(kind==='video'){const live=await searchVkVideoLive(q,page);if(live?.posts?.length)return live}
